...
struct ConditionPairInformation {
  ConditionPairInformation(Value lhs, Value rhs) {
    peelConjunction(lhs, lhsAtoms);
    peelConjunction(rhs, rhsAtoms);
  ...
  // Unique values in the first condition.
  llvm::SmallSetVector<Value, 4> lhsAtoms;
  // Unique values in the second condition.
  llvm::SmallSetVector<Value, 4> rhsAtoms;
...
private:
  ...
  // This function tries to merge two if operations.
  sv::IfOp tryToMergeIfOps(sv::IfOp ifOp, sv::IfOp prevIfOp) {
  ...
  for (auto it = body.begin(), end = body.end(); it != end; it++) {
    ...
    if (auto ifop = dyn_cast<sv::IfOp>(op)) {
      if (auto prevIf = dyn_cast_or_null<sv::IfOp>(lastSideEffectingOp)) {
        auto mergedIfOp = tryToMergeIfOps(ifop, prevIf);
